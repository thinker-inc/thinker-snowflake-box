# resource "aws_s3_bucket" "terraform_state" {
#   bucket = "terraform-state-snowflake-aws"

#   tags = {
#     Name = "terraform-state-snowflake-aws"
#   }
# }

resource "aws_dynamodb_table" "terraform_state_lock" {
  name         = "common-aws-terraform-state-lock"
  hash_key     = "LockID"
  billing_mode = "PAY_PER_REQUEST" // On-demandモード

  attribute {
    name = "LockID"
    type = "S"
  }
}

resource "aws_iam_policy" "terraform_state" {

  name        = lower("common-aws-terraform-terraform-state-policy")
  description = "Policy for Terraform state"
  policy = templatefile("${path.module}/templates/terraform_state_iam_policy.json", {
    bucket_name = "terraform-state-snowflake-aws",
    table_name  = aws_dynamodb_table.terraform_state_lock.name
  })
}
